...
# {
    ...
    if (need.issubset(school_rates.columns)) :
    # {
        None
    # }
    ...
    # {
        ...
        return print(False, "Task 3 — School Test & Proficiency Rates", ["Missing columns:", missing])
    # }
    ...
    roster_n = (roster_clean.groupby("school", dropna=False)["student_id"].nunique().rename("roster_n"))
    tested_n = (tested_ids.groupby("school", dropna=False)["student_id"].nunique().rename("tested_n"))
    ...
    ref = (pd.concat([roster_n, tested_n], axis=1).fillna({"tested_n": 0}).assign(spring_test_rate=lambda d: d["tested_n"]/d["roster_n"]).drop(columns=["roster_n","tested_n"]).join(prof_rate, how="left").fillna({"spring_prof_rate": 0}).reset_index())
    ...
    if (missing) :
    # {
        tips.append("Missing schools in candidate:", missing)
    # }
    ...
    if (extra) :
    # {
        tips.append("Unexpected schools in candidate:", extra)
    # }
    ...
    for col in ["spring_test_rate","spring_prof_rate"] :
    # {
        diff = (merged[col, "_cand"] - merged[col, "_ref"]).abs()
        bad_mask = diff > tol

        if (bad_mask.any()) :
        # {
            # Keep a few examples to help the candidate see what's off
            sample = merged.loc[bad_mask, ["school", col._"cand", col, "_ref"]].head(5)
    ...
    # {
        ...
        # {
            tips.append(col, "differs beyond tol=", tol, ". Examples:\n", sample.to_string(index=False))
        # }
    ...
    print(passed, "Task 3 — School Test & Proficiency Rates", tips)
# }
