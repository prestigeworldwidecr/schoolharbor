...
def _make_dupe_df(base_df, key=["student_id","subject","season"], date="test_date", n_dupes=300):
    ...
    if not np.issubdtype(base_df[date].dtype, np.datetime64):
    ...
    for c in base_df.columns:
        if c not in key+[date] and base_df[c].dtype.kind in "if":
...
def check_task6_strict(select_latest, assessments: pd.DataFrame):
    passed, tips = True, []
    key = ["student_id","subject","season"]; date = "test_date"
    ...
    ref = (test_df.sort_values(key + [date])
    ...
    if len(out) != expected_n:
        passed = False; tips.append(f"Output has wrong number of rows. Expected {expected_n}, got {len(out)}.")

    # 3) must be unique by key
    if out.duplicated(key).any():
        passed = False; tips.append("Output still has duplicate keys after select_latest.")

    # 4) must pick the *latest* date for each key
    merged = out.merge(ref[key+[date]], on=key, how="outer", suffixes=("", "_ref"))
    if merged.isna().any().any():
        passed = False; tips.append("Key coverage mismatch vs expected latest rows (NaNs after join).")
    bad = (merged[date] != merged[f"{date}_ref"]).sum()
    if bad > 0:
        passed = False; tips.append(f"{bad} keys are not selecting the latest {date}.")
